...
**작성일**: 2026-03-07
**분석 대상**:
- `web-output/교육안_QA_루브릭.md`
- `web-output/wb01_lecture_slide_루브릭.md`
- `web-output/wb01_remaining_루브릭.md`
**참조 파일**: `web-output/웹프로그래밍(E트랙)_교육안.md`
...
**현상**
`wb01_lecture_slide_루브릭.md` (파트 A, 65점)와 `wb01_remaining_루브릭.md` (파트 A, 50점)가 동일 파일인 `lecture-script.md`를 각각 독립적으로 평가하고 있다. 두 루브릭 모두 PASS 기준에 파트 A 하한을 요구하므로, lecture-script.md 하나로 두 번의 판정을 받아야 하는 구조가 되었다.

**원인**
K트랙 `py0N_remaining_루브릭.md` 패턴을 그대로 가져오면서 E트랙 전용 `lecture_slide` 루브릭과 역할 분리가 이루어지지 않았다.

**해결**
- `wb01_lecture_slide_루브릭.md` → lecture.md + lecture-script.md + slides.md 전담 (변경 없음)
- `wb01_remaining_루브릭.md` → **lecture-script.md 제거**, handout.md + course-notice.md 전담으로 재편
...
**현상**
`교육안_QA_루브릭.md` 영역 1-2는 "100% 한국어 작성"을 3점 만점 기준으로 삼는다. 그러나 현재 `웹프로그래밍(E트랙)_교육안.md`는 전체가 한국어/영어 Bilingual로 작성되어 있어, 루브릭 적용 시 1-2에서 자동으로 1점밖에 받지 못한다.
...
**현상**
`교육안_QA_루브릭.md` 영역 4-4는 `[S1]`, `[S2]`, `[S3]` 태그가 명확히 분리·태깅되어 있는지를 평가한다. 그러나 실제 교육안의 주차 테이블은 `**[이론]**`, `**[조작]**`, `**[AI리뷰]**` 3파트로 구성되어 있다.
...
| 루브릭 파일 | 변경 내용 |
|:------------|:---------|
| `교육안_QA_루브릭.md` | 영역 3 시험 주간 예외 추가, 영역 4-4 태그명 교체 |
| `wb01_lecture_slide_루브릭.md` | 역할 명시 주석 추가 (lecture-script 단독 담당 명시) |
| `wb01_remaining_루브릭.md` | lecture-script 파트 A 제거, handout+course-notice 전담으로 재편, 배점 재조정 |
