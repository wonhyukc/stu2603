# How to Use: 교안 및 주차 산출물 제작 가이드

이 문서는 stu2603 프로젝트에서 교안(커리큘럼)과 주차별 산출물을 제작할 때
사용하는 프롬프트·스킬·커맨드를 정리한 실무 참고서입니다.
...
[주차별 산출물 제작]
  ...
  review-quality 에이전트로 형식·용어 검증
      ↓
  /qa-py-materials py0N  ← 내용 채점 (85점 기준)
      ↓ FAIL
  /ralph-loop ← 자동 보완 반복 → PASS
```
...
| `python-output/컴퓨팅적사고와코딩(K트랙)_교육안.md` | K트랙 커리큘럼 마스터 |
| `python-output/교육안_QA_루브릭.md` | K트랙 교육안 채점 기준 |
| `python-output/py0N_remaining_루브릭.md` | 주차별 산출물 채점 기준 |
...
```
K트랙 교육안을 새로 작성해줘.
파일 경로: python-output/컴퓨팅적사고와코딩(K트랙)_교육안.md

작성 기준:
- CLAUDE.md의 핵심 제약 조건 전부 준수
- 대상: 컴퓨터 교육 처음 받는 1학년 (100% 온라인 비대면)
- 1~16주차 전체 커리큘럼, 주차별로 학습 목표·강의 내용(S1/S2/S3)·마이크로 과제 포함
...
```
python-output/컴퓨팅적사고와코딩(K트랙)_교육안.md 의 3주차 내용을 수정해줘.
[수정 내용 구체적으로 기술]
...
**대상 파일**: `python-output/컴퓨팅적사고와코딩(K트랙)_교육안.md`
**루브릭 파일**: `python-output/교육안_QA_루브릭.md`
...
```
python-output/컴퓨팅적사고와코딩(K트랙)_교육안.md 를 /qa-ktrack 으로 평가해.
...
**경로 패턴**: `python-output/py0N_remaining_루브릭.md`

py02 루브릭을 참고 템플릿으로 삼아 새 주차 루브릭을 만들 때:

```
python-output/py02_remaining_루브릭.md 를 참고해서
py03 용 루브릭을 python-output/py03_remaining_루브릭.md 로 생성해줘.
...
```
python-output/컴퓨팅적사고와코딩(K트랙)_교육안.md 의 3주차 내용을 보고
...
### 2-3. 형식·용어 검증 — `review-quality`

**용도**: 용어 통일, 분량 규정(75분·4,500단어), Bilingual 규칙, 금지 용어 스캔
**에이전트**: `.agents/skills/review-quality/SKILL.md`
**보고서 저장**: `tmp/health-report/YYYY-MM-DD_py0N_quality_report.md`

호출 방법:

```
review-quality 에이전트로 python-output/py03 폴더를 검증해줘.
```

특정 파일만 검증:

```
review-quality 에이전트로 python-output/py03/lecture-script.md 의
...
**루브릭**: `python-output/py0N_remaining_루브릭.md`
**교차 검증**: `python-output/컴퓨팅적사고와코딩(K트랙)_교육안.md` 해당 주차와 대조
...
| `review-quality` | `.agents/skills/review-quality/SKILL.md` | 형식·용어·분량 검증 |
...
| `audit-tooling` | `.agents/skills/audit-tooling/SKILL.md` | 에코시스템·폴더 구조 감사 |

### 루브릭 파일

| 파일 | 대상 | PASS 기준 |
|:-----|:-----|:---------|
| `python-output/교육안_QA_루브릭.md` | K트랙 교육안 | 90점 |
| `python-output/py0N_remaining_루브릭.md` | 주차별 산출물 4종 | 85점 (파트별 하한 동시 충족) |
...
[ ] 2. 루브릭 파일 생성
        → python-output/py03_remaining_루브릭.md
...
[ ] 4. 형식 검증
        → "review-quality 에이전트로 python-output/py03 검증해줘"
...
```
py0N 폴더가 없어. 교육안의 N주차 내용을 기반으로
build-week py0N 으로 전체 산출물을 만들고,
이어서 /qa-py-materials py0N 으로 채점해줘.
...
```
python-output/py0N/lecture-script.md 가 루브릭 A-3 기준을 못 충족해.
교육안 N주차 S2 내용을 다시 읽고 lecture-script.md 의 Section 2 를 전면 재작성해줘.
재작성 후 /qa-py-materials py0N 으로 A파트 점수 변화를 확인해줘.
```
