...
*(Instructor Note: 라이브 코딩 창에서 코드를 쳐서 적용되는 순간 화면이 변하는지 보여줍니다.)*
...
그런데 아직은 메뉴들이 왼쪽 구석에 너무 따닥따닥 붙어서 몰려 있네요. 좀 넓게 배치해 볼까요?
같은 괄호 안에 `justify-content: space-around;` 라고 코드를 한 줄 더 추가해 보겠습니다.

*(Instructor Note: space-around 적용 순간 화면의 변화를 지목합니다.)*

보셨나요? 마법처럼 메뉴들 사이의 공간이 공평하게 벌어졌습니다! 여러분이 브라우저 창의 크기를 마우스로 잡고 늘렸다 줄였다 해보세요. 브라우저 크기에 맞춰서 메뉴들 사이의 간격이 자동으로 늘어났다 줄어났다 하는 게 보이시죠? 이렇게 요소들을 고무줄처럼 유연하게(Flexible) 배치할 수 있기 때문에 이 기술의 이름이 Flexbox인 것입니다!

이번에는 `align-items: center;`도 한 줄 추가해봅시다. 지금 당장은 눈에 크게 차이가 안 날 수도 있습니다. 왜냐하면 우리 네비게이션 바의 높이가 아직 콘텐츠 높이에 딱 맞게 맞춰져 있기 때문입니다. 하지만 나중에 네비게이션 바에 `height: 60px;` 같은 고정 높이를 주거나, 로고 이미지와 텍스트 메뉴가 함께 있을 때 높이가 달라지면, `align-items: center`가 없으면 요소들이 위아래로 제각각 튀어나와 보기 좋지 않게 됩니다. 지금부터 좋은 습관을 들여 놓으세요.

이번 실습 코드를 정리하자면, 이 4줄이 바로 여러분이 앞으로 네비게이션 바나 가로 정렬이 필요한 모든 곳에 붙여 넣게 될 **Flexbox 4줄 마법 공식**입니다.
...
자, 이 4줄을 암기하실 필요는 없습니다. 언제든 떠올리기만 하면 됩니다. "가로로 나열하고 싶다? Flex 쓰면 된다." 이 한 문장만 기억하세요. 나머지는 검색하거나 AI에게 물어보면 됩니다. 우리가 매번 새로운 코드를 외울 필요는 없어요. 중요한 건 "어떤 문제를 해결하기 위해 이 도구를 쓴다"는 개념을 아는 것입니다.

조금 더 나아가서, Flexbox가 단순히 메뉴바만을 위한 기술이 아니라는 걸 보여드리겠습니다. 이 4줄을 카드 목록에 적용하면 상품 카드들을 가로로 나란히 배치할 수 있습니다. 프로필 페이지에서 사진과 소개 글을 나란히 놓을 때도 쓸 수 있습니다. 버튼을 가운데에 딱 맞추고 싶을 때도, 아이콘과 텍스트를 나란히 놓고 싶을 때도 Flexbox입니다. 현대 웹 페이지의 레이아웃 중 상당 부분은 Flexbox로 만들어집니다. 이 기술 하나를 제대로 익히면 웹 페이지의 모양을 자유자재로 제어하는 능력이 비약적으로 올라갑니다.

여러분은 이제 Flexbox를 활용해서 어떤 웹 페이지에서든 상단 네비게이션 메뉴바를 자유롭게 조립할 수 있게 되었습니다! 여러분이 만든 명함이나 자기소개 페이지에 당장 적용해 볼 수 있겠죠?
...
많은 분들이 폴더를 만들고, 폴더 안에 또 폴더를 만들고, 그 안에 또 폴더를 만들어 파일을 정리하려고 합니다. "학교 > 2026 > 1학기 > 웹기초 > 7주차 > 실습" 같은 구조가 대표적입니다. 그런데 이렇게 하면 문제가 생깁니다. 다음 주가 되면 "이 파일이 7주차 폴더에 있나, 아니면 8주차에 있나?" 하고 헷갈리거든요. 폴더 구조가 복잡해질수록 파일을 찾는 시간은 오히려 늘어납니다.
...
노트북 화면 하나에서 StackBlitz와 핸드아웃을 동시에 봐야 할 때, 창 크기를 수동으로 조절하고 계신가요? 마우스로 창의 가장자리를 잡고 늘렸다 줄였다 하는 그 번거로운 작업을 매번 반복하고 계신 건 아닌가요?
