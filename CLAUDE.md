...
> **규칙 (마스터 문서)**: 트랙에 무관하게 마스터 교육안(`*-curriculum.md`)과 루브릭(`*-rubric.md`) 파일은 기획의 밀도를 높이기 위해 **100% 한국어 단일 언어**로만 작성하며 절대로 영어 번역을 혼용하지 않는다.
> **규칙 (자료 선정)**: 모든 강의 및 실습 자료(아티클, 영상 등)는 학생이 유료 결제나 로그인을 하지 않고도 접근 가능한 **무료 오픈 소스 및 오픈 미디어** 기반으로 선정한다. 특히 **Medium**은 페이월 문제로 기피하며, BBC, NPR, TED, Wikipedia 등 공신력 있는 무료 사이트를 우선 활용한다.
