...
| 항목 | 값 |
|---|---|
| K트랙 문서 이름 | **정원혁** (직함 사용 금지) |
| E트랙 문서 이름 | **Wonhyuk William Chung** |
| 이메일 (공통) | **wonhyukc@stu.ac.kr** |

모든 문서에서 강사 이름·이메일 플레이스홀더(`[강사 이름]` 등)를 남기지 않고 위 값을 자동 삽입한다. **강사를 언급할 때는 어떠한 경우에도 "정원혁 교수님" 또는 "정원혁 교수" 등 직함을 붙이지 말고, 오직 "정원혁"으로만 완벽히 통일해야 한다.**
...
- **eCampus 세션 쿠키 관리**: eCampus 관련 스크립트 실행 중 세션 쿠키가 만료된 경우, 에이전트는 절대 "불가능하다"거나 "직접 해달라"고 응답해서는 안 됩니다. 즉시 터미널에서 `python scripts/ecampus/login_cli.py`를 실행하여 사용자가 CLI를 통해 아이디와 비밀번호를 입력하고 세션을 새로 갱신(저장)할 수 있도록 진행 상황을 이어가야 합니다.
...
- **어투 (Tone) 규칙 (공통)**: 학생들을 향한 지시문이나 문항 등을 작성할 때는 딱딱한 "~하시오." 대신 부드러운 "~하세요." 어미를 사용합니다.
