...
- **100% 대면 수업 (Flipped Learning)**: 이제 모든 수업은 대면으로 진행됩니다. 학생들은 수업 전에 온라인 녹화 영상을 먼저 시청하고 오는 **거꾸로 학습(Flipped Learning)**을 전제로 모든 가이드·스크립트(현장 진행용) 및 자료를 작성합니다.
...
- **과제 제출 (eCampus 제출 금지)**: 당분간 학생들의 eCampus 과제 직접 제출은 전면 금지된다. 모든 메인 실습 과제는 구글 폼, 서브 마이크로과제는 이메일로 제출받는다.
...
- **md 무결성 검증 필수**: Markdown(.md) 파일 수정/생성 시, 파일 내에 실수로 남은 `[강사 이름]` 등의 플레이스홀더가 없는지, 코드 블록(```) 등 문법이 깨지지 않았는지 반드시 자체 확인(예: `verify-md` 워크플로우 실행)해야 합니다.
...
| 초안~QA | 강사용 문서 | `lecture.md`, `lecture-script.md`(E트랙), `slides.md` | **한국어** |
